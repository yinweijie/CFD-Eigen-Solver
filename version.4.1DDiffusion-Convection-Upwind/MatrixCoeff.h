...
inline void MatrixCoeff::initMatrix(const Mesh* mesh)
{
    ...
    Sp = VectorXd::Zero(N);
    Sp[0] = -(2 * DA_L[0] + std::max(F_l[0], 0.0)); // 左边界引入系数矩阵的不规则项
    Sp[N-1] = -(2 * DA_R[N-1] + std::max(-F_r[N-1], 0.0)); // 右边界引入系数矩阵的不规则项 

    aP = aL + aR + (F_r - F_l) - Sp;

    Su = S_bar * V;
    Su[0] = T_A*(2*DA_L[0] + std::max(F_l[0], 0.0)) + S_bar * V[0];
    Su[N-1] = T_B*(2*DA_R[N-1] + std::max(-F_r[0], 0.0)) + S_bar * V[N-1];
